 ...
 *				interrupt				     * 
 *===========================================================================*/
PUBLIC void interrupt(task) int task; /* number of task to be started */
{
 ...
 *				sys_call				     * 
 *===========================================================================*/
PUBLIC int sys_call(function, src_dest, m_ptr) int function; /* SEND, RECEIVE, or BOTH */
 ...
 *				mini_send				     * 
 *===========================================================================*/
PRIVATE int mini_send(caller_ptr, dest, m_ptr) register struct proc *caller_ptr; /* who is trying to send a message? */
 ...
 *				mini_rec				     * 
 *===========================================================================*/
PRIVATE int mini_rec(caller_ptr, src, m_ptr) register struct proc *caller_ptr; /* process trying to get message */
 ...
 */
    ...
    /*  Gera uma prioridade aleatória para o novo processo*/
    rp->proc_priority = rand() % 10;
    ...
    if (istaskp(rp))
    {
        if (rdy_head[TASK_Q] != NIL_PROC){
            ...
            /*  CASO 1 TASK
            *   Se o novo processo tiver prioridade maior que a cabeça, 
            *   então novo processo se torna a cabeça
            *   OBS: quanto menor o proc_priority, mais prioridade
            */
           if(rdy_head[TASK_Q]->proc_priority > rp->proc_priority){

               /*   novo processo entra na primeira posicao */
               rp->p_nextready = rdy_head[TASK_Q];

               /*   cabeca se torna o novo processo */
               rdy_head[TASK_Q] = rp;
           }
           else {

                /*   cria ponteiro para apontar para cabeça da lista ligada  */
                struct proc *temp = rdy_head[TASK_Q];

                /*   navega do começo para o final da lista até encontrar
                 *   um processo com prioridade menor do que o novo processo.
                */
                while(temp->p_nextready != NIL_PROC &&
                 temp->p_nextready->proc_priority < rp->proc_priority){
                    
                    /*  vai para o próximo processo da lista    */
                    temp = temp->p_nextready;
                }

                /*  depois que encontrou um processo com menor prioridade que o novo,
                 *  insere o novo processo antes do que tem menor prioridade
                 */
                rp->p_nextready = temp->p_nextready;
                temp->p_nextready = rp;
                
                /*  caso novo processo tenha a menor prioridade,
                 *  ele é inserido no final da lista e, portanto,
                 *  devemos apontar o cauda da lista para o novo processo
                 */
            ...
            /*  CASO 2 TASK
             *  caso não tenha nenhum processo na fila,
             *  a cabeça e a cauda da lista receberão a novo processo
            */
    ...
    { /* others are similar */
        if (rdy_head[SERVER_Q] != NIL_PROC){
            ...
            /*  MESMO PROCEDIMENTO USADO NA TASK É APLICADO PARA SERVER
            *   CASO 1 SERVER
            *   Se o novo processo tiver prioridade maior que a cabeça, 
            *   então novo processo se torna a cabeça
            *   OBS: quanto menor o proc_priority, mais prioridade
            */
           if(rdy_head[SERVER_Q]->proc_priority > rp->proc_priority){

               /*   novo processo entra na primeira posicao */
               rp->p_nextready = rdy_head[SERVER_Q];
               ...
               rdy_head[SERVER_Q] = rp;
                ...
                /*   cria ponteiro para apontar para cabeça da lista ligada  */
                struct proc *temp = rdy_head[SERVER_Q];
             ...
             *  a cabeça e a cauda da lista receberão a novo processo
            */
            rdy_head[SERVER_Q] = rdy_tail[SERVER_Q]  = rp; /* add to empty queue */
            rp->p_nextready = NIL_PROC; /* new entry has no successor */
        }
        return;
    }
   ...
   */
    if (rdy_head[USER_Q] == NIL_PROC){
            ...
            /*  MESMO PROCEDIMENTO USADO NA TASK É APLICADO PARA USER
            *   CASO 1 USER
            *   Se o novo processo tiver prioridade maior que a cabeça, 
            *   então novo processo se torna a cabeça
            *   OBS: quanto menor o proc_priority, mais prioridade
            */
           if(rdy_head[USER_Q]->proc_priority > rp->proc_priority){
               ...
               rdy_head[USER_Q] = rp;
                ...
                /*   cria ponteiro para apontar para cabeça da lista ligada  */
                struct proc *temp = rdy_head[USER_Q];
 ...
 *				unready					     * 
 *===========================================================================*/
PRIVATE void unready(rp) register struct proc *rp; /* this process is no longer runnable */
{
 ...
 *				lock_mini_send				    *
 *==========================================================================*/
PUBLIC int lock_mini_send(caller_ptr, dest, m_ptr) struct proc *caller_ptr; /* who is trying to send a message? */
int dest;                                                                   /* to whom is message being sent? */
message *m_ptr;                                                             /* pointer to message buffer */
{
 ...
 *				lock_ready				    *
 *==========================================================================*/
PUBLIC void lock_ready(rp) struct proc *rp; /* this process is now runnable */
{
 ...
 *				lock_unready				    *
 *==========================================================================*/
PUBLIC void lock_unready(rp) struct proc *rp; /* this process is no longer runnable */
{
 ...
 *				cp_mess					    *
 *==========================================================================*/
PRIVATE void cp_mess(src, src_p, src_m, dst_p, dst_m) int src; /* sender process */
...
message *dst_m;                                                /* destination buffer */
{
    ...
    src_m = (message *)((char *)src_m + (((phys_bytes)src_p->p_map[D].mem_phys - src_p->p_map[D].mem_vir) << CLICK_SHIFT));
    dst_m = (message *)((char *)dst_m + (((phys_bytes)dst_p->p_map[D].mem_phys - dst_p->p_map[D].mem_vir) << CLICK_SHIFT));
#ifdef NEEDFSTRUCOPY
